...
const buildTransactionSchemaWithWarnings = (
  args: GenerateTransactionBuilderParams,
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
) =>
  TransactionSchema.transform(async (txn) => {
    const warnings: string[] = [];

    const { totalCost, totalFT, totalNear } = await calculateTransactionCosts(
      txn,
      args.nearNetworkId,
    );

    if (totalCost > WARNING_PRICE) {
      // TODO: remove or pass logger explicitly?
      // logAgentWarning(
      //   'DEFAULT',
...
export const generateTransactionPrimitive: BitteToolBuilder<
...
> = (args: GenerateTransactionBuilderParams) => ({
  ...
  execute: async ({
    transactions,
    network = 'mainnet',
  }): Promise<BitteToolResult<GenerateTransactionResult>> => {
    const validatedTransactions: z.infer<
      ReturnType<typeof buildTransactionSchemaWithWarnings>
    >[] = [];
    ...
    try {
      ...
      for (const transaction of transactions) {
        const validatedTransaction =
          await buildTransactionSchemaWithWarnings(args).parseAsync(
            transaction,
          );
      ...
      case 'FunctionCall':
        if (FINANCIAL_TRANSACTION_METHODS.includes(action.params.methodName)) {
          const ftMetadata = FTS_METADATA.find(
            (ft) => transaction.receiverId === ft[nearNetworkId],
          );
